...
public class Algorithms {
    ...
    public static void fixConvexity(List<Point2D> points,OrientationEnum orientation){
        ...
        while (true){
            ...
            //body su nespravne 
            Point2D tmp;
            iterator.previous();//presun medzi cur a next vracia next
            iterator.previous();//presun medzi prev a cur vracia cur
            iterator.remove(); // zmaze cur
            
            iterator.previous();//presun pred prev
            
            if (!iterator.hasPrevious()){
                iterator.next();//presun za prev
                cur = iterator.next();//presun za cur
                if (!iterator.hasNext()){
                    break;
                }
                next = iterator.next();
